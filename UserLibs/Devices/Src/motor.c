...
#define MOTOR_PID_MAX_IOUT 0.0

fp32 pid_k[3]={0.0,0.0,0.0};
...
void Move(Motor *self, uint8_t isBrake, fp32 setRPM) {
    if (isBrake) {
      ...
      fp32 PWM_OUT = self->pid.out;
        if (setRPM > 0) {
            // 正转：IN1 高、IN2 低 | Forward: IN1 high, IN2 low
            __HAL_TIM_SET_COMPARE(self->Init.htim, self->Init.Channel_1, fabsf(PWM_OUT));
            ...
            __HAL_TIM_SET_COMPARE(self->Init.htim, self->Init.Channel_2, fabsf(PWM_OUT));
